...
- Staged-governance mode: CI runs telemetry report generation (`must_not_exist`, non-blocking) + governance alignment.
- Enforcement (`must_not_exist`) is available as `make cyber-hygiene-enforce` and is enabled only when repo is ready.
...
## Merge decision
**READY FOR MERGE (staged-governance route)** when:
- `make cyber-hygiene-full` is green,
- report generation is successful,
- governance contract check is green,
- `dropped_invalid_rows: 0`.

**Enforcement hard-gate enablement criteria**:
- switch CI to `make cyber-hygiene-enforce`,
- require `status: "PASS"`,
- require `mode: "must_not_exist"`,
- require `present_disallowed_classes: []`.
