...
Pinned ≠ safe code; a pinned SHA can still contain a vulnerability. SHA
resolution was done once via the GitHub API and recorded; it is not
re-resolved each run (that would re-introduce a network trust point).
Re-pinning to newer SHAs is a deliberate, reviewed change.
