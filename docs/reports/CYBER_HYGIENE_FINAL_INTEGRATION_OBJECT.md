...
1. CI executes `cyber-hygiene-full` with policy mode `must_not_exist`.
...
The gate must return **FAIL** if any of the following is true:

- Any targeted class is present in `must_not_exist` mode.
