...
| Unverifiable theorem prose | A private claim asserted but never tested | Theorem must become a pinned spec + probe before any belief update; engine refuses narrative-only input | The engine cannot judge whether the redaction is faithful | `docs/PRIVATE_RND_PROTOCOL.md`, `examples/indi_redacted_cognitive_time.yaml` |
