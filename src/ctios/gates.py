"""Release gate. Fail-closed. Never auto-softens a RED verdict."""
...
def evaluate_gate(
    ...
    I = agg["injected"]
    best_naive = min(
        (v["post_shift_mae"] for k, v in agg.items() if k.startswith(("moving_average", "last_interval", "exp_smoothing"))),
        default=float("inf"),
    )
    c: dict[str, bool] = {}
